...
pub fn response_review_list_page(
    ...
    let items = if details.is_empty() {
        r#"<p class="text-base-content/70">No responses are waiting for your review.</p>"#.into()
    } else {
        details
            .iter()
            .map(|detail| {
                format!(
                    r#"<li class="rounded-xl border border-base-300 p-4">
                        <a class="text-primary hover:text-base-content font-semibold" href="/reviews/{id}?company_id={company_id}">{subject}</a>
                        <p class="mt-1 text-sm text-base-content/70">Version {version} · expires {expires}</p>
                    </li>"#,
                    id = detail.draft.id,
    ...
    base_layout(
        ...
        &format!(
            r#"<header class="mb-6"><h1 class="text-2xl font-bold">Response reviews</h1>
                <p class="text-base-content/70">Pending responses assigned to you.</p></header>
...
pub fn response_review_detail_page(detail: &ResponseReviewDetail) -> String {
    ...
    let recipients = match &detail.draft.recipients {
        DraftRecipientSnapshot::V1 { to, cc } => {
            ...
            let cc = cc
                ...
                .join(", ");
            format!(
                "<p><span class=\"text-base-content/70\">To:</span> {to}</p><p><span class=\"text-base-content/70\">Cc:</span> {cc}</p>"
            )
        }
    };
    ...
    let evidence = if detail.evidence.is_empty() {
        "<li class=\"text-base-content/70\">No evidence references were retained.</li>".into()
    } else {
        detail.evidence.iter().map(|item| {
            ...
            format!(r#"<li class="rounded-lg border border-base-300 p-3">
                <p>{source} <span class="text-xs text-base-content/70">{support} · {availability}</span></p>
                <p class="text-xs text-base-content">{warning}</p></li>"#,
    ...
    let attachments = if detail.draft.attachments.is_empty() {
        "<li class=\"text-base-content/70\">No attachments.</li>".into()
    } else {
        detail
            .draft
            .attachments
            ...
            .map(|attachment| {
                format!(
                    "<li>{} <span class=\"text-xs text-base-content/70\">{}</span></li>",
    ...
    let decision = match (
        ...
        (Some(feedback), _) => format!(
            "<p class=\"my-4 rounded-lg border border-error/40 p-3\"><strong>Feedback:</strong> {}</p>",
            escape_html_text(feedback),
        ),
        (_, Some(rationale)) => format!(
            "<p class=\"my-4 rounded-lg border border-base-300 p-3\"><strong>Decision rationale:</strong> {}</p>",
    ...
    let content = format!(
        r##"<a class="text-sm text-primary" href="/reviews?company_id={company_id}">← All reviews</a>
        <a class="ml-4 text-sm text-primary" href="/reviews/policy?company_id={company_id}&amp;channel_id={channel_id}">Review policy</a>
        <header class="my-5"><h1 class="text-2xl font-bold">{subject}</h1>
            <p class="text-base-content/70">Draft version {version} · {status}</p></header>
        <section class="space-y-1 rounded-xl border border-base-300 p-4">{recipients}
            <p><span class="text-base-content/70">Destination:</span> {destination}</p></section>
        <section class="my-5"><h2 class="font-semibold mb-2">Exact draft</h2>
            <pre class="whitespace-pre-wrap rounded-xl bg-base-300 p-4 text-sm">{body}</pre></section>
        ...
        <ul class="my-4 text-sm text-base-content">{warnings}</ul>
        {decision}
        <section class="my-5"><h2 class="font-semibold mb-2">Version history</h2><ul class="text-sm text-base-content/70">{history}</ul></section>
        <div id="review-result"></div>
        <form method="post" action="/reviews/edit" hx-post="/reviews/edit" hx-target="#review-result" hx-disabled-elt="find button[type='submit']" class="my-5 space-y-2 rounded-xl border border-primary/40 p-4">
          ...
          <input class="w-full rounded-lg bg-base-300 p-2" name="subject" value="{edit_subject}" required>
          <label class="block text-sm">To</label>
          <input class="w-full rounded-lg bg-base-300 p-2" name="recipient_to" value="{edit_to}" required>
          <label class="block text-sm">Cc (comma separated)</label>
          <input class="w-full rounded-lg bg-base-300 p-2" name="recipients_cc" value="{edit_cc}">
          <label class="block text-sm">Response</label>
          <textarea class="w-full rounded-lg bg-base-300 p-2" name="body" required>{edit_body}</textarea>
          <button class="rounded-lg bg-primary px-4 py-2 font-semibold" type="submit">Save new version</button>
        </form>
        <div class="grid gap-4 md:grid-cols-2">
          <form method="post" action="/reviews/approve" hx-post="/reviews/approve" hx-target="#review-result" hx-disabled-elt="find button[type='submit']" class="space-y-2 rounded-xl border border-success/40 p-4">
            ...
            <textarea class="w-full rounded-lg bg-base-300 p-2" name="rationale" maxlength="2048"></textarea>
            <button class="rounded-lg bg-success px-4 py-2 font-semibold" type="submit">Approve and publish</button>
          </form>
          <form method="post" action="/reviews/reject" hx-post="/reviews/reject" hx-target="#review-result" hx-disabled-elt="find button[type='submit']" class="space-y-2 rounded-xl border border-error/40 p-4">
            {common}<input type="hidden" name="command_id" value="{reject_command}">
            <label class="block text-sm">Feedback</label>
            <textarea class="w-full rounded-lg bg-base-300 p-2" name="feedback" maxlength="8192" required></textarea>
            <button class="rounded-lg bg-error/10 px-4 py-2 font-semibold" type="submit">Reject</button>
          </form>
          <form method="post" action="/reviews/reassign" hx-post="/reviews/reassign" hx-target="#review-result" hx-disabled-elt="find button[type='submit']" class="space-y-2 rounded-xl border border-base-300 p-4">
            {common}<input type="hidden" name="command_id" value="{reassign_command}">
            <label class="block text-sm">New reviewer principal ID</label>
            <input class="w-full rounded-lg bg-base-300 p-2" name="reviewer_principal_id" required>
            <button class="rounded-lg bg-base-300 px-4 py-2 font-semibold" type="submit">Reassign</button>
          </form>
        </div>"##,
        company_id = detail.draft.company_id,
...
pub fn response_review_action_result(message: &str, company_id: uuid::Uuid) -> String {
    format!(
        r#"<div class="rounded-xl border border-success/40 bg-success/10 p-4 text-base-content">{}
...
pub fn response_review_policy_page(
    ...
    let body = format!(
        r#"<header class="mb-6"><h1 class="text-2xl font-bold">Response review policy</h1>
           <p class="text-base-content/70">Effective policy: {effective}</p></header>
           <div id="policy-result"></div>
           <form method="post" action="/reviews/policy/company" class="mb-5 space-y-3 rounded-xl border border-base-300 p-4">
             <input type="hidden" name="company_id" value="{company_id}">
             <input type="hidden" name="channel_id" value="{channel_id}">
             <label class="block font-semibold">Company default</label>
             <select name="policy" class="rounded-lg bg-base-300 p-2">
             ...
             <button class="rounded-lg bg-primary px-4 py-2" type="submit">Save company default</button>
           </form>
           <form method="post" action="/reviews/policy/channel" class="space-y-3 rounded-xl border border-base-300 p-4">
             <input type="hidden" name="company_id" value="{company_id}">
             <input type="hidden" name="channel_id" value="{channel_id}">
             <label class="block font-semibold">Channel override</label>
             <select name="policy_override" class="rounded-lg bg-base-300 p-2">
             ...
             <input name="preferred_reviewer_principal_id" value="{preferred}" class="w-full rounded-lg bg-base-300 p-2">
             <button class="rounded-lg bg-primary px-4 py-2" type="submit">Save channel policy</button>
           </form>"#,
...
pub fn response_review_policy_saved(company_id: uuid::Uuid, channel_id: uuid::Uuid) -> String {
    format!(
        r#"<div class="rounded-xl border border-success/40 p-4">Policy saved.
