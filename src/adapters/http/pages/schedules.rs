...
pub fn schedules_page(page: &SchedulesPage<'_>) -> String {
    ...
    let content = format!(
        ...
        <div id="schedules-workspace" class="flex flex-1 min-w-0">
            {runs_html}
            {pane_html}
        </div>
        "##,
...
pub fn schedules_empty_pane(message: &str, swap: FragmentSwap) -> String {
    format!(
        r##"
        <section id="schedule-pane"{PANE_SKELETON} class="flex flex-1 items-center justify-center bg-base-100 p-8"{oob}>
